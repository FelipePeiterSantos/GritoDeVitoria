...
function scene:createScene( event )
	group = self.view
	blackscene = display.newImage("telaCastelhano.png",display.contentWidth/2,display.contentHeight/2)
...
function scene:enterScene( event )
	...
	timer.performWithDelay(5000,function() transition.to(group,{time=2000,alpha=0,onComplete=function() storyboard.gotoScene("stage3_1") end}) end)
end
function scene:exitScene( event )
	...
	timer.performWithDelay(1,function () storyboard.purgeScene("loadStage3") end)
end
